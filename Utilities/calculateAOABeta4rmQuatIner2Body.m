function [AOA,Sideslip,UAOA,USideslip] = calculateAOABeta4rmQuatIner2Body(state,P_vec,...
                                         vi,ve,qi,qe,wind_vel,lat,lon,primemeridian,omega)
...
Vel_veh = state(:,vi:ve);
Vel_veh(:,2) = Vel_veh(:,2) - omega.*cos(state(:,2)).*state(:,1);
Quat = state(:,qi:qe);
...
for ii = 1:1:num_t
   ...
   R_iner2cruise = R_veh2body_wq(q0,q1,q2,q3);
   R_Cru2Body = [1 0 0;0 0 -1;0 1 0];
   R_iner2vehmat = R_iner2veh(rad2deg(lat(ii)),rad2deg(lon(ii)+primemeridian(ii)));
   R_veh2body = R_Cru2Body*R_iner2cruise*R_iner2vehmat';

function [x2estimate, X2estimate, Uncertainty, P2estimate,zk,Kk,Qk,...
    qk_list,Deltaq_k_list,qk_mean_list,Qk_list,...
    l_list,vk_list,Deltav_k_list,vk_mean_list,Vk_list] = AdaptiveFilter_wMeasCovariance(t2,X1,P1,t1,...
    measurements,HMatrixFcnHandle,RMatrixFcnHandle,...
    inputs,error_level,kk,N,qk_list,Deltaq_k_list,qk_mean_list,Qk_list,Qk,...
    l_list,L,vk_list,Deltav_k_list,vk_mean_list,Vk_list,type)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% State propogation
len_state = length(X1);         % Size of state vector
...
% Initialize parameters for iteration to calculate process noise covariance
delX = 1;
iter = 1;
iter_max = 1;
iter_tol = 1e-4;

% Iteration until process noise converges
while delX > iter_tol && iter <= iter_max
    %% Kalman update (i.e. measurement update)
    % Calculate measurement sensitivity matrix and measurement residual
    try
    ...
    %---------------------------------------------------------------------
    % Adaptive filtering measurmeent
    % Calculate measurement noise matrix
    % What is the batch number currently for the measurement
    ll = l_list(type);
    
    % Surrogate for measurement noise vector
    vk = zk;
    Deltav_k = Hk*P2nominal*Hk';
    
    % Store calculated values for future use
    vk_list{type,ll} = vk;
    Deltav_k_list{type,ll} = Deltav_k;
    
    % Calculate measurement noise for next step -- using Myers/Tapley cumulation
    try
        if ll > L
            l_list(type) = l_list(type) + 1;
            vk_mean = vk_mean_list{type,ll-1};
            vk_mean = vk_mean + (1/L).*(vk - vk_list{type,ll-L});
            Rk = Vk_list{type,ll-1};
            Rk = Rk + (1/(L-1)).*((vk-vk_mean)*(vk-vk_mean)'-(vk_list{type,ll-L}-vk_mean)*(vk_list{type,ll-L}-vk_mean)'+...
                + (1/L).*(vk - vk_list{type,ll-L})*(vk - vk_list{type,ll-L})' + ((L-1)/L).*(Deltav_k_list{type,ll-L}-Deltav_k));
            Rkprime = Rk;
        else
            l_list(type) = l_list(type) + 1;
            Rk = RMatrixFcnHandle(t2,X2nominal,measurements,inputs,error_level);
            vk_mean = zeros(length(measurements),1); % Save zeros when batch size still not met
            
            if ll ~= 1
                if ll ~= L
                    vk_mean = vk_mean_list{type,ll-1};
                    vk_mean = (vk_mean*(ll-1) + vk)/ll;
                    Rkprime = zeros(length(measurements));
                else
                    vk_mean = vk_mean_list{type,ll-1};
                    vk_mean = (vk_mean *(ll-1)+vk)/ll;
                    Rkprime = zeros(length(measurements));
                    for ii = 1:L
                        Rkprime = (vk - vk_mean)*(vk - vk_mean)' - ((L-1)/L)*Deltav_k;
                    end
                    Rkprime = (1/(L-1)).*Rkprime;
                end
            else
                vk_mean = vk;
                Rkprime = zeros(length(measurements));
    ...
    % Check for positive definiteness of the R-matrix
    [~,eigenval] = eig(Rkprime);
    eigenval_vec = diag(eigenval);
    ind_neg_eig = find(eigenval_vec<=0,1);
    if isempty(ind_neg_eig)~=1
        Rk_calc = Rkprime;
        for uu = 1:length(eigenval_vec)
            Rkprime(uu,uu) = abs(Rk_calc(uu,uu));
        end
    end
    
    % Save mean and measurement noise covariance calculated
    vk_mean_list{type,ll} = vk_mean;
    Vk_list{type,ll} = Rkprime;
    ...
    disp(['Current time: ',num2str(t2),' sec. Condition number: ',num2str(cond_num),'. ',num2str(kala),'Num Obs 1,2, 3 :',num2str(l_list')]);
...
% Save mean and process noise covariance calculated
